...
export function ChunkNavigation({ meetingId, prevChunk, nextChunk }: ChunkNavigationProps) {
  return (
    <nav className="grid grid-cols-2 gap-3 pt-6 mt-6 border-t border-border/50" aria-label="Chunk navigation">
      {prevChunk ? (
        <Link
          href={`/meetings/${meetingId}/chunks/${prevChunk.id}`}
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <div className="text-sm font-medium text-foreground truncate">{prevChunk.title}</div>
          </div>
        </Link>
      ) : (
        <div />
      ...
      {nextChunk ? (
        <Link
          href={`/meetings/${meetingId}/chunks/${nextChunk.id}`}
        ...
        >
          <div className="min-w-0">
            ...
            <div className="text-sm font-medium text-foreground truncate">{nextChunk.title}</div>
          </div>
          <div className="shrink-0 w-8 h-8 rounded-full bg-muted flex items-center justify-center group-hover:bg-primary/10 transition-colors">
